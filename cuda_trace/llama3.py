...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0):
    ...
    # dummy embeddings
    freqs_cis = torch.ones_like(freqs_cis, dtype=torch.bfloat16)
    return freqs_cis
...
class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: ModelArgs, freqs_cis: torch.Tensor):
        super().__init__()
        ...
        self.freqs_cis = nn.Parameter(freqs_cis)
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        for layer_id in range(params.n_layers):
            self.layers.append(TransformerBlock(layer_id, params, freqs_cis))
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int):
        ...
        for layer in self.layers:
            h = layer(h, start_pos, mask, seqlen)
        h = self.norm(h)
